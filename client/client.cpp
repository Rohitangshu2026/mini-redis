// Command-line client for the mini-redis wire protocol.
//
// Two modes, chosen by argc:
//   one-shot:     ./mini-redis-client set foo bar     (argv is the command)
//   interactive:  ./mini-redis-client                 (a REPL on stdin)
//
// Unlike the server, the client uses plain blocking I/O — it only ever has
// one request in flight, so read_full/write_all are all it needs.
...
// Open a blocking TCP connection to ip:port.
// Returns the connected fd, or -1 with the failure already printed.
...
// Pretty-print one serialized value from the reply, redis-cli style:
// strings quoted, integers/doubles/errors labelled, arrays indented one
// level per nesting depth. Recurses for array elements.
// Returns the number of bytes consumed, or -1 if the value is truncated —
// every length is checked against what's actually in the buffer before use.
...
// Encode one command as [u32 nstr]([u32 len][bytes])*, frame it, send it,
// then read and print the framed typed reply.
// Returns 0 on success, -1 on any transport or framing problem.
...
// Split an input line on whitespace into command arguments.
// (No quoting rules — an argument can't contain spaces in the REPL.)
