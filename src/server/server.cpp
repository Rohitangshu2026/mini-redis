// The server core: accept loop, per-connection buffered I/O, and timers,
// all driven by a single poll() loop. One iteration does, in order:
//   1. build the pollfd set (listener + every live connection)
//   2. poll(), sleeping at most until the nearest timer is due
//   3. accept any new connections
//   4. for each ready connection: refresh its idle timer, then read
//      (parsing and answering every complete pipelined request) or write
//   5. reap expired connections and expired keys
// Nothing here blocks: sockets are non-blocking and slow work (freeing
// large values) is handed to the worker pool.
